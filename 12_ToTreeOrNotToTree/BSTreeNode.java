...
public class BSTree <T extends Comparable>{
    ...
    public BSTreeNode findpar(BSTreeNode a){
        return findparh(a, root);
    }
    
    public BSTreeNode findparh(BSTreeNode a, BSTreeNode b){
        if(b.getLeft() == a || b.getRight() == a){
            return b;
        }
        BSTreeNode d = null;
        BSTreeNode c = null;
        if(b.lcheck()){
            d = findparh(a , b.getLeft());
        }
        if(b.rcheck()){
            c = findparh(a, b.getRight());
        }
        if(c != null){
            return c;
        }
    ...
    private BSTreeNode restructure(BSTreeNode curr, BSTreeNode x){
        if(x == root){
            BSTreeNode tmp = x;
            if(x.rcheck()){
                tmp = tmp.getRight();
                ...
                findpar(tmp).setRight(null);
                if(x.lcheck()){
                    tmp.setLeft(x.getLeft());
                }
                if(tmp.rcheck()){
                    BSTreeNode z = tmp.getRight();
                    while(z.rcheck()){
                        z = z.getRight();
                    }
            ...
            if(x.lcheck()){
                ...
                findpar(tmp).setLeft(null);
                if(x.rcheck()){
                    tmp.setRight(x.getRight());
                }
                if(tmp.lcheck()){
                    BSTreeNode z = tmp.getLeft();
                    while(z.lcheck()){
                        z = z.getLeft();
                    }
                    z.setLeft(x.getLeft());
    ...
    private BSTreeNode<T> remove( BSTreeNode<T> curr, T c ) {
	    if(curr.lcheck() && curr.getLeft().getValue() == c){
	        if(isLeaf(curr.getLeft())){
	            curr.setLeft(null);
	            return root;
	        }
	        return restructure(curr,curr);
	    }
	    if(curr.rcheck() && curr.getRight().getValue() == c){
	        if(isLeaf(curr.getRight())){
	           curr.setRight(null);
	           return root;
	        }
	        // deal with not a leaf
	    }
	    BSTreeNode a = null;
	    BSTreeNode b = null;
	    if(curr.lcheck()){
	        a = remove(curr.getLeft(), c);
	    }
	    if(curr.rcheck()){
	        b = remove(curr.getRight(),c);
	    }
	    if(a != null && a != root){
	        return a;
	    }
